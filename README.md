...
- **Свежий Xray не проходит там, где проходит старый.** Это не сеть и не
...
### Пакет Xray и выбор версии

`packages/xray.scm` — официальный статический бинарник релиза (как
`claude-code`, но даже без patchelf).

**Версия подобрана под сеть.** С 25.7.26 клиент REALITY в сети провайдера
t1 опознаётся DPI и режется: первые 2–3 соединения проходят, дальше TCP
до сервера устанавливается, но данные не идут (`Send-Q` растёт, Claude Code
отваливается по таймауту). 25.6.8 и старше работают. Блокировка **залипает
на несколько минут**, поэтому одиночный запрос ничего не показывает.

Проверять новую версию так — временный экземпляр на своём порту, рабочий
сервис не трогаем:
