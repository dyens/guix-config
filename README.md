...
Списка ключей в коде нет: `home/ssh.scm` и `home/kube.scm` перечисляют
ключи прямо из зашифрованного файла (`home/secrets.scm`). В sops
шифруются только **значения**, имена ключей открыты. Добавили ключ в
секрет — `homerec`, и он на машине; пока список был в коде, он уже
подвёл — ключ `ruclaw-test-deploy` лежал в секрете и никуда не приезжал.

Подкаталог в `/run/user/<uid>/secrets/` задаётся **именем ключа**:
sops-guix склеивает части ключа через `/`. То есть ключ, названный

```
ssh/croc-gitlab
```

расшифруется в `/run/user/<uid>/secrets/ssh/croc-gitlab`. YAML при этом
остаётся плоским. Переименовать ключи можно в любой момент:

```sh
guix shell sops -- sops files/secrets/ssh.yaml   # ssh/<имя> вместо <имя>
```

Правок в коде не нужно и тут: пути строятся из имени ключа, так что
конфигурация верна и до переименования, и после.
...
содержимое `skills`, `commands` и `hooks` сам. Списка файлов там нет
намеренно: пока он был, скилл `debug-ruclaw-trace` лежал в репозитории и
не приезжал на машину, потому что строчку дописать забыли.
