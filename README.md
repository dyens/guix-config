...
печатает `\a` — это делает хук в `files/bashrc` на трапе `DEBUG`. Дальше:

- tmux (`monitor-bell on`) красит вкладку в статусбаре, пока в неё не заглянешь;
- `bell-action other` пробрасывает сам звонок в локальный терминал за ssh,
  но только если вы сейчас не в этом окне. `any` — звенеть всегда,
  `none` — молчать и ограничиться цветом вкладки.
