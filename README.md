...
### Оповещение о завершении долгой команды

Команда, шедшая дольше `DY_BELL_SEC` секунд (по умолчанию 20), на выходе
подаёт **два** сигнала — см. хук в `files/bashrc`:

1. **`\a` в `/dev/tty`** — сигнал, который работает всегда. Его ловит
   tmux (`monitor-bell on`), красит вкладку красным, пока в неё не
   заглянешь, и показывает в статусбаре `Bell in window N`
   (`visual-bell both`). Через ssh он тоже проходит.
2. **Подробная строка в статусбаре**: `готово: sleep 4 (4с)` или
   `ошибка 1: make -j8 (95с)` — её шлёт сам bashrc через
   `tmux display-message`. Работает, **только если `$TMUX` в этом шелле
   не пуст**, то есть команда идёт на той же машине, где запущен tmux.

#### Где tmux, там и настройки

Ключевой момент, на котором легко потерять полдня. Если tmux запущен на
рабочей машине, а команды вы гоняете на VM по ssh, то:

- `files/tmux.conf` должен быть применён **на рабочей машине**, а не на VM
  — вкладки, звонок и `visual-bell` разбирает тот tmux, что рисует экран;
- `files/bashrc` нужен **на VM** — это он замеряет время и печатает `\a`;
- подробной строки «готово: …» в этом раскладе не будет: в удалённом
  шелле `$TMUX` пуст. Останется `Bell in window N` и красная вкладка.


Время замеряется через `PS0`, а не через трап `DEBUG`: `PS0`
разворачивается ровно один раз на введённую команду, а `DEBUG`
срабатывает перед каждым звеном `PROMPT_COMMAND` (там ещё `_direnv_hook`)
— с ним в длительность попадало время, пока вы думали над приглашением.

#### Как проверить

Главное: **проверять надо из другого окна**. `bell-action other` намеренно
молчит, если звонок пришёл из того окна, на которое вы сейчас смотрите.

```sh
# в окне 2:
DY_BELL_SEC=2      # понизить порог, чтобы не ждать
sleep 4
# тут же C-a 1 — уйти в окно 1 и подождать
```

Через 4 секунды: в статусбаре появится `готово: sleep 4 (4с)`, а вкладка
`2` покраснеет и останется красной, пока в неё не зайдёшь.

Если ничего нет — проверять по слоям:

| Что проверить | Команда | Ожидается |
|---|---|---|
| хук вообще загружен | `type _dy_bell_precmd` | `_dy_bell_precmd is a function` |
| он в цепочке | `echo "$PROMPT_COMMAND"` | начинается с `_dy_bell_precmd` |
| засечка стоит | `echo "$PS0"` | непустая строка с `_dy_started` |
| tmux перечитал конфиг | `tmux show -g bell-action` | `bell-action other` |
| он покажет звонок | `tmux show -g visual-bell` | `visual-bell both` |
| окна следят за звонком | `tmux show -wg monitor-bell` | `monitor-bell on` |

Последние три проверяются на той машине, где запущен tmux, первые три —
там, где выполняется команда.

Чаще всего забыто одно из трёх: `homerec` (без него `~/.bashrc` — старый
симлинк в стор, нужен ещё и новый шелл: `exec bash`), `C-a r` в уже
запущенном tmux-сервере, и — самое коварное — конфиг применён на VM, а
tmux работает на рабочей машине, где его никто не обновлял.

#### Звука нет — так и задумано

Стоковый `st` из Guix собран с `bellvolume = 0`: звука он не издаёт
вообще. Всё, что он делает с `\a`, — ставит своему окну флаг urgent, и то
лишь если окно не в фокусе (в i3 это подсветка в панели). Поэтому сигнал
номер 1, строка в статусбаре, и есть рабочий, а на `\a` рассчитывать не
стоит. Нужен именно звук — придётся пересобирать `st` с другим
`bellvolume`.

#### Что не сигналит

Меряется время всей командной строки, поэтому `vim файл`, в котором вы
просидели три минуты, без оговорок давал бы «готово: vim файл (180с)» —
шум, вы и так смотрели в это окно. Интерактивные программы исключены
списком `DY_BELL_SKIP` (`vim`, `emacs`, `less`, `man`, `top`, `ssh`,
`watch`, …). Сравнивается первое слово команды, без пути.

```sh
DY_BELL_SKIP="$DY_BELL_SKIP mycmd"   # добавить в этом шелле
```

Правится насовсем — в `files/bashrc`, там же и порог.

#### Ручки

```sh
ding make -j8          # запустить и позвонить, порог не важен
make -j8; ding         # то же хвостом
DY_BELL_SEC=60         # поднять порог в шумном окне
DY_BELL_SEC=0          # выключить автозвонок в текущем шелле
```

Звонок печатается в `/dev/tty`, а не в stdout: иначе `долгая_команда > log`
уронил бы `\a` в лог.
...
заглядывать к нему каждые полминуты, на событие `Stop` (ответ закончен,
ждёт ввода) вешается `files/claude/hooks/notify.sh`. Он делает два дела:

- пишет в статусбар `claude: готов · guix-config` — это и есть сигнал
  по смыслу, штатное «Bell in window N» сообщает только факт;
- звонит `\a`, а дальше работает уже настроенное в `files/tmux.conf`:
...
- **У хука Claude Code может не быть `/dev/tty`.** Его запускает сам
  ...
  `tmux display-message -p -t "$TMUX_PANE" '#{pane_tty}'`. `TMUX` и
  `TMUX_PANE` в окружении хука есть, проверено.
- **`local-file` теряет бит `+x`** у одиночного файла. Для скрипта нужен
  `#:recursive? #t`, иначе хук молча не запустится.
