...
- **`lsm=` заменяет список модулей безопасности, а не дополняет.** В ядре
  ...
  Проверка: `dmesg | grep landlock` → `Up and running`. Файл
  `/sys/kernel/security/lsm` появляется только при смонтированном
  `securityfs`; в `%base-file-systems` его нет, в `t1.scm` он добавлен.
