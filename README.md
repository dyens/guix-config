...
Самый простой способ управлять секретом — не иметь его. Пара ключей
генерируется **на каждой машине отдельно**, приватная половина никогда
её не покидает, в репозиторий едет только публичная:

```sh
ssh-keygen -t ed25519 -C "dyens@$(hostname)" -f ~/.ssh/id_ed25519
cat ~/.ssh/id_ed25519.pub > files/keys/dyens-$(hostname).pub
```

`#:ssh-authorized-keys` принимает список, так что ключей может быть
сколько угодно — по одному на машину:
...
Выгода не только в удобстве: скомпрометированную машину вы отзываете
удалением одного файла и `reconfigure`, а не сменой единственного ключа
всюду.

#### Токены: `pass`

После предыдущего пункта из секретов остаются переменные окружения —
`GITLAB_TOKEN`, `JIRA_API_TOKEN`, kubeconfig. Их держит `password-store`:
GPG-шифрование плюс собственный git-репозиторий.

`password-store`, `gnupg` и `home-gpg-agent-service-type` уже прописаны
в `home/dyens.scm`; pinentry — `pinentry-tty`, чтобы ввод парольной фразы
работал и в ssh-сессии без X.

Первая настройка:

```sh
gpg --full-generate-key            # если GPG-ключа ещё нет
pass init <gpg-key-id>
pass git remote add origin git@github.com:dyens/guix-secrets.git
pass insert gitlab/token
pass git push -u origin master
```

На новой машине: привезли GPG-ключ руками (один раз — это граница схемы,
а не её недостаток), затем `pass git clone`.

Использование в шелле, в неуправляемом `~/.bashrc.local`:

```sh
export GITLAB_TOKEN=$(pass show gitlab/token)
```

`pass show` дёрнет gpg-agent при каждом новом шелле; кэш парольной фразы
поднят до часа. Если и это мешает — держите токены прямо в
`~/.bashrc.local` (права 600), а `pass` используйте как долговременный
архив, откуда наполняете этот файл на новой машине.

**Секреты, даже зашифрованные, не кладите в `guix-config`.** Публичный
шифротекст выкачивают и хранят: ключ может утечь через год, а копия
блоба останется у всех. `pass` для того и держит отдельный приватный
репозиторий.
