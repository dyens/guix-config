...
1. **Строка в статусбаре tmux**: `готово: sleep 4 (4с)` или
   `ошибка 1: make -j8 (95с)`. Видна сразу, в каком бы окне вы ни сидели,
   и ни от какого терминала не зависит. Это основной сигнал.
2. **`\a` в `/dev/tty`**. Его ловит tmux (`monitor-bell on`) и красит
   вкладку в статусбаре красным, пока в неё не заглянешь, а `bell-action
   other` пробрасывает звонок дальше, в локальный терминал за ssh.
...
Чаще всего забыто одно из двух: `homerec` (без него `~/.bashrc` — старый
симлинк в стор, нужен ещё и новый шелл: `exec bash`) и `C-a r` в уже
запущенном tmux-сервере.
