...
(define-module (systems wg-quick)
  ...
  #:use-module (gnu packages bash)              ; sh для перенаправления
  #:use-module (gnu packages dns)               ; openresolv (resolvconf)
...
(define (wg-quick-program name user)
  (program-file
   ...
   #~(begin
       ...
       ;; DNS из конфига wg-quick ставит через resolvconf (openresolv). Свежий
       ;; openresolv не трогает /etc/resolv.conf, написанный не им (dhcpcd
       ;; пишет его сам): «signature mismatch … run `resolvconf -u`», и
       ;; wg-quick up падает. Поэтому отдаём файл openresolv: текущие серверы
       ;; DHCP — отдельной записью dhcp.orig, и -u пересобирает файл с его
       ;; подписью. При down openresolv вернёт серверы dhcp.orig.
       (define (adopt-resolv-conf)
         (let ((line (call-with-input-file "/etc/resolv.conf" read-line)))
           (unless (and (string? line)
                        (string=? line "# Generated by resolvconf"))
             (log "handing /etc/resolv.conf over to resolvconf")
             (system* #$(file-append bash-minimal "/bin/sh") "-c"
                      (string-append #$(file-append openresolv "/sbin/resolvconf")
                                     " -a dhcp.orig < /etc/resolv.conf; "
                                     #$(file-append openresolv "/sbin/resolvconf")
                                     " -u")))))
       (when (file-exists? "/etc/resolv.conf")
         (adopt-resolv-conf))
